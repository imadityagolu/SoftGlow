...
const Product = () => {
    ...
    const fetchFeedback = async () => {
      if (product && product.id) {
        try {
          ...
          const response = await feedbackService.getFeedbackByProduct(product.id);
          if (response.success) {
  ...
  const renderStars = (rating) => {
    ...
    const numericRating = Math.max(0, Math.min(5, Math.floor(Number(rating) || 0)));
    
    return Array.from({ length: 5 }, (_, i) => {
      const isFilled = i < numericRating;
      return (
        <span key={i} className={`text-lg ${isFilled ? 'text-yellow-400' : 'text-gray-300'}`}>
          {isFilled ? '★' : '☆'}
        </span>
      );
    });
            ...
            <div>
              ...
              <div className="flex items-center space-x-2 mb-4">
                <div className="flex items-center">
                  {renderStars(feedbackStats?.averageRating || product.rating)}
                </div>
                <span className="text-gray-600">({feedbackStats?.averageRating?.toFixed(1) || product.rating || '5.0'})</span>
              </div>
              ...
              <div className="flex items-center space-x-4 mb-4">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-600">
                    Based on {feedbackStats.totalRatings} review{feedbackStats.totalRatings !== 1 ? 's' : ''}
                  </div>
                </div>
              </div>
            </div>
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <div>
                          ...
                          <div className="items-center space-x-2">
                            ...
                            <br/>
                            {renderFeedbackStars(review.rating)}
                          </div>
                        </div>
                      </div>
              ...
              {feedback.length > 3 && (
                <div className="text-center py-4">
                  ...
                  <p className="text-orange-600 text-sm font-medium">
                    + {feedback.length - 3} more reviews available
                  </p>
                </div>
              )}
            </div>
